...
def collect():
    ...
    try:
        sql = '''SELECT b.BookID,
                        b.BookTitle,
                        b.BookAuthor,
                        floor((b.PubilcationYear-1000)/10) FROM (SELECT * from Collects) a  
                        LEFT  JOIN  Books as b on a.BookID = b.BookID where a.UserID = "{}"
                        '''.format(userid)
        cat = mysql.fetchall_db(sql)
        cat = [[v for k, v in row.items()] for row in cat]
    except Exception as e:
        logger.exception("historical rating books error: {}".format(e))
...
@app.route("/addcart_collect", methods=['POST', 'GET'])
def add_collect():
    '''
    添加收藏
    '''
    ...
    try:
        if request.method == 'GET':
            ...
            sql = '''SELECT COUNT(1) as count FROM Collects WHERE UserID="{0}" and BookID="{1}" '''.format(userid,
                                                                                                       bookid)
            count = mysql.fetchone_db(sql)
            if not count['count']:
                sql = '''INSERT INTO Collects (UserID,BookID ) values ('{0}','{1}') '''.format(userid, bookid)
                mysql.exe(sql)
                logger.info("update Collects  success,sql:{}".format(sql))
    except Exception as e:
        logger.exception("update Collects  books error: {}".format(e))
    return Response(status=204)

@app.route("/delete_collect", methods=['POST', 'GET'])
def delete_collect():
    '''
    删除收藏
    '''
    ...
    try:
        if request.method == 'GET':
            bookid = request.values.get('bookid')
            sql = '''DELETE  FROM Collects WHERE UserID="{0}" and BookID="{1}" '''.format(userid,bookid)
            mysql.exe(sql)
            logger.info("delete Collects  success,sql:{}".format(sql))
    except Exception as e:
        logger.exception("delete Collects  books error: {}".format(e))
